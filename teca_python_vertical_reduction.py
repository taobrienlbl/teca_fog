...
class teca_python_vertical_reduction(teca.teca_python_algorithm):
    """
    The base class used for writing new vertical reduction algorithms in Python.
    Contains plumbing that connects user provided overrides to an instance of
    teca_vertical_reduction. Users are expected to override one or more of
    ...
    def execute(self, port, input_data, request):
        ...
        # get the input mesh
        in_mesh = teca.as_const_teca_cartesian_mesh(input_data[0])

        # construct the output
        out_mesh = teca.as_const_teca_cartesian_mesh(in_mesh.new_instance())

        # copy metadata
        out_mesh.copy_metadata(in_mesh)

        # fix the metadata
        out_md = teca.teca_metadata(out_mesh.get_metadata())
        ...
        # fix the z axis
        cart_mesh = teca.as_const_teca_cartesian_mesh(out_mesh)
